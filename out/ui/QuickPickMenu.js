...
class QuickPickMenu {
    ...
    async reauthenticate(profileId) {
        ...
        if (user.username.toLowerCase() !== profile.githubUsername.toLowerCase()) {
            ...
            const choice = await vscode.window.showWarningMessage(`This profile says @${profile.githubUsername}, but GitHub verified @${user.username}. Update this profile to the verified username? Its project mappings will be preserved.`, { modal: true }, 'Update Profile');
