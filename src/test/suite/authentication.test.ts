...
suite('Saved authentication and repository assignment', () => {
    ...
    test('explicit switch works with auto-switch disabled; failed setup preserves selection', async () => {
        ...
        try {
            ...
            BrowserAuthStrategy.prototype.loginViaBrowser = async () => ({ username: 'work', email: '', displayName: '', accessToken: 'wrong-token' });
            await assert.rejects(() => bridgeHandler!({ action: 'reauthenticate', id: oauthId }), /already belongs to 'Work'/);
            assert.strictEqual(savedSecrets.get(`github_token_${oauthId}`), 'replacement-token');
            HTTPSAuthStrategy.prototype.configureHTTPSAuth = async () => true;
