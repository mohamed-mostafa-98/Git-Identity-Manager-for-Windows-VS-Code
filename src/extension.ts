...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('githubAccountManager.reauthenticate', async (profileId?: string) => {
            try {
                if (!profileId) {
                    const selected = await vscode.window.showQuickPick(
                        profileManager.getProfiles()
                            .filter(profile => profile.authenticationMethod === AuthenticationMethod.BROWSER_OAUTH)
                            .map(profile => ({ label: profile.displayName, description: `@${profile.githubUsername}`, id: profile.id })),
                        { title: 'Reauthenticate Browser Account', placeHolder: 'Choose the saved account to repair' }
                    );
            ...
            const bridge = await startDesktopBridge(vscode.workspace.name || 'VS Code — no folder', async request => {
                ...
                if (request.action !== 'snapshot') {
                    ...
                    switch (request.action) {
                        ...
                        case 'reauthenticate': await quickPickMenu.reauthenticate(request.id!); break;
