...
export class TokenHealthService {
    private request(host: string, route: string, token: string): Promise<Response> {
        return new Promise((resolve, reject) => {
            const request = https.get({ hostname: host, path: route, headers: {
                ...
                response.on('data', chunk => {
                    ...
                    if (host === 'api.github.com') body += chunk;
    ...
    async check(profile: AccountProfile, token: string | undefined, repository?: RepositoryDetails): Promise<string[]> {
        ...
        const failure = (response: Response): string => {
            ...
            if (response.status === 401) return 'Token rejected: it may be expired, revoked or invalid. Replace it or sign in again.';
            if (response.status === 403) return 'Access denied (403). Check token repository selection, write permissions and organization policies.';
            if (response.status === 404) return 'Repository unavailable (404): it may not exist, or this token may lack access.';
        ...
        if (user.status !== 200) return [failure(user)];
        let login: unknown;
        try { login = JSON.parse(user.body).login; } catch { /* handled below */ }
        if (typeof login !== 'string' || !/^[A-Za-z0-9-]+$/.test(login)) return ['GitHub returned an unexpected account response. Retry.'];
        if (login.toLowerCase() !== profile.githubUsername.toLowerCase()) return [`Wrong account: token belongs to @${login}, but this profile expects @${profile.githubUsername}. Update the saved token.`];
        const lines = [`Token accepted for @${login}.`];
        if (!repository) return [...lines, 'Open a Git repository to check its access.'];
        ...
        if (repo.status !== 200) return [...lines, `${name}: ${failure(repo)}`];
        ...
        lines.push('Branch rules and workflow-file permissions may still reject a particular push.',
            'This checks the extension’s saved token. Terminal Git may use a different cached credential; use “Switch Account Profile” to apply this token to the project.');
        if (repository.isSSH) lines.push('This repository uses SSH; terminal pushes use its SSH key, not this token.');
        return lines;
    }

    async repositoryMetadata(profile: AccountProfile, token: string | undefined, repository: RepositoryDetails) {
        if (!token || !isValidToken(token)) throw new Error('This mapped account needs valid saved authentication.');
        if (repository.hostname !== 'github.com') throw new Error('Agent repository access supports GitHub.com only.');
        const user = await this.request('api.github.com', '/user', token);
        let login: unknown;
        try { login = JSON.parse(user.body).login; } catch { /* handled below */ }
        ...
        if (response.status !== 200) throw new Error(`GitHub repository access failed with HTTP ${response.status}.`);
        let value: any;
        try { value = JSON.parse(response.body); } catch { throw new Error('GitHub returned invalid repository metadata.'); }
        return {
